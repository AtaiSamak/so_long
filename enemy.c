...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/08 22:46:50 by bahsoka           #+#    #+#             */
/*   Updated: 2021/10/08 22:46:52 by bahsoka          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	whereMoveEnemy(t_map *map)
{
	...
	|| map->map[map->ePosY + 1][map->ePosX] == 'P'))
		return ('S');
	else if (map->posY < map->ePosY && \
	(map->map[map->ePosY - 1][map->ePosX] == '0' || \
	map->map[map->ePosY - 1][map->ePosX] == 'P'))
		return ('W');
	else if (map->posX > map->ePosX && \
	(map->map[map->ePosY][map->ePosX + 1] == '0' || \
...
void	moveEnemy(t_map *map, int y, int x)
{
	...
	map->bg, map->ePosX * map->blockSize, map->ePosY * map->blockSize);
	map->ePosX = x;
	map->ePosY = y;
	...
	mlx_put_image_to_window(map->mlx, map->win, \
	map->enemy, map->ePosX * map->blockSize, map->ePosY * map->blockSize);
}
